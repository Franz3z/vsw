...
@app.route('/complete_task/<group_id>/<task_id>', methods=['POST'])
def complete_task(group_id, task_id):
    try:
        ...
        if file:
            ...
            file.save(file_path)
            # Update task as completed and save file reference
            task_ref = db.reference(f'groups/{group_id}/tasks/{task_id}')
            task_ref.update({'completed': True, 'uploaded_file': filename})
            flash('Task marked as complete and file uploaded!')
            return redirect(request.referrer)
    except Exception as e:
        logging.error(f"Error completing task {task_id}: {e}")
        flash('An error occurred while completing the task.')
        return redirect(request.referrer)
