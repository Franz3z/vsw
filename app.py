...
def get_completed_tasks(group_id):
    ...
    try:
        ...
        for task_id, task in completed_tasks_data.items():
            ...
            progress_reports = task.get('progress_reports', {})
            if progress_reports:
                latest_report_id = sorted(progress_reports, key=lambda k: progress_reports[k]['timestamp'], reverse=True)[0]
                latest_report = progress_reports[latest_report_id]
                task['progress'] = latest_report.get('progress', 'N/A')
                task['file_name'] = latest_report.get('file_name', None)
